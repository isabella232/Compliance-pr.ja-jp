...
- **[データ主体の要求（DSR）](gdpr-data-subject-requests.md)**。個人データに関するアクション (変更、制限、アクセス) を実行するように、データ主体が管理者に対して行う正式な要求。
- **[セキュリティ違反の通知](gdpr-breach-notification.md)**。GDPR における個人データ違反とは、「送信、保存、またはその他の方法で処理される個人データの偶発的または違法の破損、損失、改変、不正漏洩、またはアクセスを引き起こすセキュリティ違反」のことです。
...
GDPR では、個人 (またはデータ主体) に対して、個人データの処理に関連した特定の権利が付与されます。これには、不正確なデータを修正する権利、データを抹消する権利、処理を制限する権利、自分のデータを受け取る権利、および自分のデータを別の管理者に送信するという要求を実行する権利が含まれます。管理者には、タイムリーで一貫性のある GDPR 応答を提供する責任があります。技術的な詳細については、「[データ主体の要求](gdpr-data-subject-requests.md)」を参照してください。  
...
GDPR における管理者は、個人の権利と自由に対して高いリスクをもたらす可能性のあるデータ処理 (特に新しいテクノロジを使用した処理) を実行する前に、DPIA を実施する必要があります。GDPR には、どのような場合に DPIA を実行する必要があるかに関して、その一部を示した次のリストが用意されています。
...
Microsoft は、GDPR アカウンタビリティをサポートするツールやドキュメントを提供しています。これには、データの件名権のサポート、独自のデータ保護影響評価の実行、共同作業を行って個人のデータ違反が解決されます。
...
GDPR では、EUにお住いの方は「データ主体の権利」のセットを使用して個人データを制御できます。これには次のものが含まれます。
...
規制で識別されたいくつかの要因によって決まります。GDPR の第 37 条により、管理者と処理者は次のいずれかの場合、データ保護責任者を指定する必要があります。(a) 公的機関が処理を行う場合（司法権を有する裁判所を除く）. (b) 管理者または処理者の主要な活動が、その性質、範囲、および/または目的により、データ対象者を大規模に定期的かつ体系的に監視することを必要とする処理業務から構成される場合。(c) 管理者または処理者の中核的な活動が、第 9 条に基づく特別な種類のデータおよび第 10 条に言及する刑事上の有罪判決と犯罪に関する個人データを大規模に処理することで構成されている場合。
...
GDPR では、個人データの違反があったときに、そのデータを判読不能にする保護手段として暗号化を認定しています。 そのため、暗号化が使用されているかどうかは、個人データ違反の通知要件に影響する可能性があります。 GDPR では、暗号化を、一部のリスクに対処する適切な技術的または組織的手段としても指摘しています。 暗号化は、クレジット カード業界 (PCI) データ セキュリティ スタンダード (DSS) による要件でもあり、金融サービス業界に固有の厳格なコンプライアンス ガイドラインの一部でもあります。 Azure、Dynamics 365、Enterprise Mobility + Security、Office Microsoft 365、SQL Server/Azure SQL データベース、Windows 10、Windows 11 などの Microsoft の製品とサービスでは、転送中のデータと保存中のデータに対して堅牢な暗号化を提供します。
